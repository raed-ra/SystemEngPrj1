...
int check_return(char *line, FILE *c_file, char *name_register, int *inside_function, int *main_printed, int *start_w_tab, FILE *ml_file, int *line_number, int arg_c, char *arg_v[]) {
    if (strncmp(line, "return", 6) == 0 && isspace(line[6])) {
        ...
        while (token != NULL) {
            ...
            if (!identifier_exists(name_register, token)) {
                fprintf(stderr, "!Error: Invalid or no parameter name '%s' at line %d Instead of assuming the identifier is zero its better to let you know that you have printed an undeclared identifier.\n", token, *line_number);
                exit(EXIT_FAILURE);
            }
            token = strtok(NULL, " ()[]+-*/");
        }
...
int check_print(char *line, FILE *c_file, int *inside_function, int *main_printed, int *start_w_tab, FILE *ml_file, int *line_number, int arg_c, char *arg_v[], char *name_register) {
    if (strncmp(line, "print", 5) == 0 && isspace(line[5])) {
        ...
        while (token != NULL) {
            ...
            if (!identifier_exists(name_register, token)) {
                fprintf(stderr, "!Error: Invalid or no identifier '%s' at line %d. Instead of assuming the identifier is zero its better to let you know that you have printed an undeclared identifier\n", token, *line_number);
                exit(EXIT_FAILURE);
            }
            token = strtok(NULL, " ,()[]+-*/");
        }
